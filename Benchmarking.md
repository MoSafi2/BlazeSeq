...
- **Data**: A single synthetic FASTQ file of **1 GB** is generated with the same format used elsewhere in BlazeSeq: 4-line records, header `@read_<index>`, read length 100 bp, Sanger-like quality (generic schema).
...
2. On **Linux**, a tmpfs is mounted on that directory (`mount -t tmpfs -o size=2G tmpfs <dir>`). The 1 GB synthetic file is written there. All reads during hyperfine then hit RAM.
3. The 1 GB file is generated by the Mojo script `benchmark/generate_synthetic_fastq.mojo` and written into that directory.
...
Install the following so that `benchmark/run_benchmarks.sh` can run:
...
- **needletail**: `benchmark/needletail_runner/Cargo.toml` (e.g. 0.6.x)
- **seq_io**: `benchmark/seq_io_runner/Cargo.toml` (e.g. 0.3.x)
- **kseq**: vendored from [lh3/seqtk](https://github.com/lh3/seqtk) (kseq.h)
- **FASTX.jl**: `benchmark/Project.toml` (FASTX dependency)

## Interpreting Results

- **Time**: hyperfine reports mean and median run time (e.g. in seconds). Lower is better.
- **Throughput**: For a 1 GB file, throughput in GB/s is `1.0 / time_seconds`. Use this to compare parsers across machines.
