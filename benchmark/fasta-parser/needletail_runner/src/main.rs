//! FASTA parser benchmark runner using needletail.
//! Reads path from argv[1], counts records and base pairs, prints "records base_pairs".
//! needletail's parse_fastx_file auto-detects FASTA and FASTQ formats.
...
fn main() {
    ...
    let mut reader = match parse_fastx_file(path) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("needletail_fasta_runner: failed to open {}: {}", path, e);
            process::exit(1);
        }
    };
    ...
    while let Some(record) = reader.next() {
        let seqrec = match record {
            Ok(r) => r,
            Err(e) => {
                eprintln!("needletail_fasta_runner: parse error: {}", e);
                process::exit(1);
            }
        };
        total_reads += 1;
