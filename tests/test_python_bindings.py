...
def _normalize_id(raw_id: str) -> str:
    """Return read id without leading '@' (extension may expose with or without)."""
...
def test_create_parser_and_next_record():
    ...
    while True:
        try:
            ...
            count += 1
            if count == 1:
                assert _normalize_id(rec.id) == "EAS54_6_R1_2_1_413_324"
...
def test_next_batch_and_get_record():
    ...
    assert _normalize_id(first.id) == "EAS54_6_R1_2_1_413_324"
    second = batch.get_record(1)
    assert _normalize_id(second.id) == "EAS54_6_R1_2_1_540_792"

    batch2 = parser.next_batch(10)
    assert batch2.num_records() == 1
    assert _normalize_id(batch2.get_record(0).id) == "EAS54_6_R1_2_1_443_348"
...
def test_parser_iterator_protocol():
    ...
    while True:
        try:
            ...
            raise
    assert len(recs) == 3
    assert _normalize_id(recs[0].id) == "EAS54_6_R1_2_1_413_324"
...
def test_batch_iterator_protocol():
    ...
    assert len(recs) == 2
    assert _normalize_id(recs[0].id) == "EAS54_6_R1_2_1_413_324"
    assert _normalize_id(recs[1].id) == "EAS54_6_R1_2_1_540_792"
